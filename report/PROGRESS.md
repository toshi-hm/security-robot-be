...
| Phase 9: Docker環境 | ✅ 完了 | 100% | Dockerfile刷新・ComposeでAPI/Redis/PostgreSQL/Celeryのヘルスチェック整備 (2025-10-24: 環境変数外部化と本番イメージ軽量化を追記 / 2025-10-25: Compose既定値とCelery並列度の可変化、DockerfileへUID/GIDビルド引数を追加 / 2025-10-27: Dockerビルド時のDNS障害に備えてbuildネットワークを`host`に指定し、APT依存を維持したままビルド成功を確認 / 2025-10-30: 開発サービスの起動コマンドから依存再インストールを撤廃し、appuser権限へ統一してボリューム共有時の所有権衝突を解消 / 2025-12-19: GPU必須構成へ戻し、CUDAベースイメージとGPUデバイス要求を既定化) |
...
- [x] app/services/template_agent_service.py / rl/utils/comparison.py - 比較APIレスポンスと進捗イベントで動的ステップ上限を反映し、`max_steps=None` 時は環境サイズ由来の上限値を返却 (2025-11-20追加)
...
- [x] 同ドキュメントへファイル管理API(`/files/upload`/`list`/`download`/`delete`)の入出力仕様とストレージ配置ルール、Celeryアーカイブタスクとの関係を記述する。(2025-10-22完了)
- [x] 環境セッション操作API(`/environment/sessions` 系)のリクエスト/レスポンスとセッション上限・タイムアウト・ロック方針を文書化し、現行実装の並行アクセス制御を設計書へ反映する。(2025-10-22完了)
...
- [x] バッテリーシステム統合 (2025-11-11完了)
  ...
  - [x] SecurityEnvironmentへのバッテリーシステム実装
    ...
    - [x] バッテリー消費・充電メカニズム（1000ステップで1%消費、1ステップで1%充電）
    - [x] 充電ステーション配置（エピソードごとにランダム位置、障害物・境界を回避）
...
- [x] GPU必須構成に戻し、CUDAベースイメージと `gpus: all` を既定化 (2025-12-19)
...
- [x] 開発ステージのDockerfileで`COPY --chown`と`USER appuser`を適用し、イメージ内のファイル所有者と実行ユーザーを統一 (2025-10-30更新)
- [x] Compose開発サービスから起動時の`uv pip install`を撤廃し、ビルド済み`.venv`を共有ボリュームで再利用 (2025-10-30更新)
- [x] 835fe97コミットで導入した`chown`レイヤー分離を検証したところ、APP_UID変更時に依存インストール層も再実行される問題を638fda3で解決（依存インストールをユーザー作成より前に移動） (2025-10-30解決)

> ℹ️ 現在の開発コンテナには Docker CLI がインストールされていないため、`docker compose config` 実行時にコマンド未検出エラーを確認。ローカル環境またはCIでの実機検証時に再実行すること。
...
- [x] 残り22個のエラーを手動修正（W293、UP035、UP031、F841、E402）
- [x] CI/CDパイプラインへのRuffチェック組み込み（GitHub Actions）
- [x] **全377個のエラーを0個へ削減完了**
...
**対応済みエラー（全22個）:**
- E402 (7件): 意図的なimport配置 → `# noqa: E402` 追加
- W293 (12件): docstring内の空白行 → 修正完了
- UP035 (1件): `typing.Dict` → `dict` へ更新
- UP031 (1件): パーセントフォーマット → f-string へ変換
- F841 (1件): 未使用変数 → `_` プレフィックス追加
...
2. **Phase 4/6フォロー**: CeleryリボークAPIなど強制停止パスの実証と、再開/再キューイング時の状態遷移設計（停止理由タイムスタンプのヘルパー化は完了。再停止時も`resumed_at`を維持する挙動に合わせて監査要件を精査し、Celery revoke統合テストを設計・実装する）
   - 2025-10-23: `playback.archive_session`タスクの実装により、成果物登録とホットストレージ削除が自動化された。次は強制停止時のアーカイブ再実行ポリシーと監査ログ連携を定義する。
   - 2025-10-24: チャンクサイズ設定・ZIPサイズ制限・Redis通知トランザクションIDを導入し、リトライ時の冪等性とDoS耐性を強化。運用ポリシーと監査ログ設計をアップデート予定。
   - 2025-10-24: 展開比率検査とバッチ削除ログを追加したため、監査ログとメトリクスでの追跡方法を整理し、閾値調整プロセスを設計する。
   - 2025-10-31: ファイルストレージの`resolve`ガードを`Path.relative_to()`に切り替えて、プレフィックス一致によるディレクトリ脱出を封じた。類似の入力検証抜けが他サービスにないか棚卸しする。
...
5. **ドキュメント整備フォロー**: ファイル管理API章立てとアーカイブ連携要件を設計書へ反映済み。環境セッション操作APIのロック・タイムアウト検証観点もテスト計画に追記した。次はJobManager監査ログとの成果物紐付け(アーカイブ完了時のファイルID記録)と、環境セッション強制終了API追加時のドキュメントテンプレート整備を検討する。
   - 2025-10-27: OpenAPIエクスポート用スタブのdocstringをリント指摘に合わせて調整し、スクリプトの可読性を向上。

### 🌟 中優先度
4. **Phase 9完了**: Docker環境の完全検証
   - 2025-10-26: BuildKit 指定を除去し、API ヘルスチェック用の Python スクリプトを導入して compose 設定の可読性と堅牢性を改善。
   - 2025-10-28: Docker ビルド時の DNS 制限に備えて build ネットワークを `host` に指定し、APT 依存の導入を維持したままビルドが成功することを確認する。
   - 2025-10-29: README と docker/README.md に `network: host` の背景とセキュリティ考慮事項を追記し、Compose 設定のコメントで意図を共有。
...
8. **バックアップ取り込み**: `scripts/import_backup_trainingdata.py` で `db_export.json` を PostgreSQL に取り込む（接続情報の確認後に実行）

### 📌 低優先度 (後回し可)
9. **運用監視**: ログ収集・分析基盤の具体化
10. **リポジトリ整備**: `.gitignore` の適用範囲を継続的に確認し、必要なら追加除外ルールを検討
