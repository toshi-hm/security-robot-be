...
5. **ジョブキューのリソース制御**
   ...
   - 同一 `session_id` を同時に操作するAPIリクエスト間の競合を防ぐため、`asyncio.Lock` 等の排他制御導入可否を評価する必要あり。
...
7. **JobManagerメモリ・排他対策**: インメモリキューの保持件数上限と排他制御の必要性を検討し、改善方針を提案
