...
### 🔥 高優先度
1. **Phase 4/6フォロー**: CeleryリボークAPIなど強制停止パスの実証と、再開/再キューイング時の状態遷移設計（停止理由タイムスタンプのヘルパー化は完了。再停止時も`resumed_at`を維持する挙動に合わせて監査要件を精査し、Celery revoke統合テストを設計・実装する）
