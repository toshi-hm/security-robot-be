...
### 🔥 高優先度
1. **Phase 4/6フォロー**: CeleryリボークAPIなど強制停止パスの実証と、再開/再キューイング時の状態遷移設計（停止後の再キュー時メタデータは整備済み、再停止時も`resumed_at`を維持する挙動に合わせて監査要件を精査中。Celery revoke統合テストが未着手）
