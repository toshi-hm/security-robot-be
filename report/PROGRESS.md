...
| Phase 8: テスト | 🔄 進行中 | 80% | 学習メトリクスAPIとファイル管理APIの単体テストを整備し、ファイルダウンロード統合テストを追加。404レスポンス検証を防御的アサーションと診断メッセージで強化 (依存不足を解消済み) / 2025-10-13: トレーニング制御のCeleryタスクID返却とジョブキューAPIを検証する統合テストを追加 / 2025-10-19: プレイバックAPIのユニット・統合テストでモジュール名衝突が発生していたため、統合テストファイルをリネームして `pytest` 全体実行を復旧 |
...
### 🔥 高優先度
1. **セッションロック本体の導入**: JobManagerへ`session_id`単位の`asyncio.Lock`マップを実装し、API呼び出し(`stop`/`resume`)がロックを介して直列化されるようにする。既存の保持ポリシーと干渉しないこと、TTLスイープやディスカード処理がロック階層と矛盾しないことを設計レビューする。
2. **セッションロック試験実装**: instructions/04_test_design_standalone.md に並行`stop`/`resume`ケースのテスト戦略を追記済み。InstrumentedLockフィクスチャと`asyncio`並列検証を実装して計画をテストコードへ反映する。
3. **JobManager保持実装フォロー**: backend API設計書にジョブキュー応答の新メタデータを反映。API単体/統合テストで`paused_at`/`resumed_at`/`forced`のシリアル化を追加検証する。
4. **Phase 4/6フォロー**: CeleryリボークAPIなど強制停止パスの実証と、再開/再キューイング時の状態遷移設計（停止理由タイムスタンプのヘルパー化は完了。再停止時も`resumed_at`を維持する挙動に合わせて監査要件を精査し、Celery revoke統合テストを設計・実装する）
5. **Phase 5→6連携**: Redis転送とジョブマネージャー状態を同期させ、WebSocketブリッジの自動監視まで仕上げる
6. **Phase 8継続**: Celeryタスクとジョブキューのエラー時ロールバックシナリオを設計し、バックプレッシャー制御を検討
