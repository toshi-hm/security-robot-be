...
5. **ジョブキューのリソース制御**
   ...
   - 2025-10-21: JobManagerへ保持上限・TTLスイープ実装と単体テスト(履歴優先パージ/アクティブ超過/TTL掃除)を追加し、ドキュメントで計画したポリシーをコード化。閾値はDI可能にしてテストで極小構成を使えるようにした。
...
### 🔥 高優先度
1. **セッションロック試験設計**: `stop`/`resume` の並行呼び出しを想定したセッション粒度ロック導入時のユニットテスト戦略を設計し、`asyncio`並列実行で順序保証を検証する。
2. **JobManager保持実装フォロー**: APIレスポンス/スキーマが新しい保持メタデータと揃っているか確認し、必要なドキュメント更新やAPIテストを洗い出す。
3. **Phase 4/6フォロー**: CeleryリボークAPIなど強制停止パスの実証と、再開/再キューイング時の状態遷移設計（停止理由タイムスタンプのヘルパー化は完了。再停止時も`resumed_at`を維持する挙動に合わせて監査要件を精査し、Celery revoke統合テストを設計・実装する）
4. **Phase 5→6連携**: Redis転送とジョブマネージャー状態を同期させ、WebSocketブリッジの自動監視まで仕上げる
5. **Phase 8継続**: Celeryタスクとジョブキューのエラー時ロールバックシナリオを設計し、バックプレッシャー制御を検討
