...
### 🔥 高優先度
1. **JobManager保持実装フォロー**: backend API設計書へ停止・再開メタデータを反映済み。次はAPI単体/統合テストで`paused_at`/`resumed_at`/`forced`のシリアル化を追加検証する。
