...
### 🔥 高優先度
1. **セッションロック試験実装**: instructions/04_test_design_standalone.md に並行`stop`/`resume`ケースのテスト戦略を追記済み。InstrumentedLockフィクスチャと`asyncio`並列検証を実装して計画をテストコードへ反映する。
2. **JobManager保持実装フォロー**: backend API設計書にジョブキュー応答の新メタデータを反映。API単体/統合テストで`paused_at`/`resumed_at`/`forced`のシリアル化を追加検証する。
