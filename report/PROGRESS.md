...
| Phase 9: Docker環境 | ✅ 完了 | 100% | Dockerfile刷新・ComposeでAPI/Redis/PostgreSQL/Celeryのヘルスチェック整備 (2025-10-24: 環境変数外部化と本番イメージ軽量化を追記 / 2025-10-25: Compose既定値とCelery並列度の可変化、DockerfileへUID/GIDビルド引数を追加 / 2025-10-27: Dockerビルド時のDNS障害に備えてbuildネットワークを`host`に指定し、APT依存を維持したままビルド成功を確認 / 2025-10-30: 開発サービスの起動コマンドから依存再インストールを撤廃し、appuser権限へ統一してボリューム共有時の所有権衝突を解消) |
