...
### 🔥 高優先度
1. **Phase 4/6フォロー**: CeleryリボークAPIなど強制停止パスの実証と、再開/再キューイング時の状態遷移設計（停止後の再キュー時メタデータは整備済み、再停止時の`resumed_at`除去も完了。Celery revoke統合テストが未着手）
