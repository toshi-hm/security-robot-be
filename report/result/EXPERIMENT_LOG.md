...
| **12** | 48 | 100k | 1.0 | 0.5 | 0.5 | **-2484** | **0.88** | 完了 (効率向上) |

---

| **11** | 47 | 100k+ | 1.0 | 0.5 | 0.5 | **-4940** | **0.89** | 完了 (効果大) |

---

## 詳細レポート

### Cycle 12: Efficiency Optimization (Dynamic Patrol Radius) (完了)

- **Job ID**: 48 (Standalone Run)
- **実施日時**: 2025-12-09
- **目的**: 脅威度に応じた「動的警備半径」の導入により、カバレッジ速度と脅威除去のバランス（効率）を最適化する。
- **仮説**:
    - 低脅威時: 半径拡大 (Exploration Mode) -> カバレッジ向上。
    - 高脅威時: 半径通常 (Suppression Mode) -> 脅威除去確実化。
- **結果**:
    - **Final Coverage**: **0.88** (Cycle 11: 0.89) - ほぼ同等、微減。
    - **Threat Level**: **0.68** (Cycle 11: 0.72) - **改善**。
    - **Reward**: **-2484** (Cycle 11: -4940) - **大幅改善 (約2倍)**。
- **考察**:
    - カバレッジは飽和状態 (0.88-0.90) で頭打ちだが、報酬が劇的に改善した。
    - これは「無駄に歩き回る」だけでなく「脅威を効率的に除去している」ことを示唆する。
    - 脅威レベルが低下したことからも、動的半径による「発見・除去能力の向上」が確認できた。
    - **結論**: Cycle 11 (Coordination) と Cycle 12 (Efficiency) の組み合わせにより、非常に高性能なエージェントが完成した。

### Cycle 11: Multi-Agent Coordination (完了)

- **Job ID**: 47 (Standalone Run)
- **実施日時**: 2025-12-09
- **目的**: **Visit History Map** (観察ch2) の追加による重複巡回抑制とカバレッジ向上 (>0.75) の検証。
- **ハイパーパラメータ**:
    - `total_timesteps`: 100,000
    - `visit_history_decay`: 500 steps (Env Param)
    - **Reward Weights**:
        - `coverage_weight`: 1.0
...
- **期待**:
    - エージェントが「最近誰かが通った場所 (青色)」を避けるようになり、自然と分散配置が実現される。
- **結果**:
    - **Final Coverage**: **0.89** (Episode 2) - **目標 (0.75) を大幅に達成**。
    - **Reward**: -4940 (改善傾向)。
    - Threat Level: 0.72 (やや高止まり)。
- **考察**:
    - Visit History Map の導入により、カバレッジが 0.73 (Cycle 08 Best) から **0.89** へと劇的に向上した。
    - ロボットは「他者(または過去の自分)が通った場所」を避けることで、自然に未踏破領域へ押し出されている。
    - 課題: カバレッジは高いが、脅威度(0.72)が下がっていない。広範囲をカバーするあまり、個々の脅威の「消火(滞在)」が疎かになっている可能性がある。
    - 次の Cycle 12 で「動的警備半径」を導入し、効率的な消火を目指す方針は妥当である。
