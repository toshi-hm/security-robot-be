...
def _record_metric(
  ...
  try:
    ...
    try:
      ...
      logger.debug(
        "Failed to close metrics session for training session %s", session_id, exc_info=True
      )
...
def _make_training_status_probe(session_id: int) -> Callable[[], TrainingJobStatus | None]:
  def _probe() -> TrainingJobStatus | None:
    session = SessionLocal()
    try:
      result = session.query(TrainingJob.status).filter(TrainingJob.id == session_id).first()
...
def run_ppo_training_task(self: Any, session_id: int, config: dict[str, Any]) -> dict[str, Any]:
  ...
  try:
    ...
    metrics_interval = _resolve_interval(config.get("metrics_update_interval"), progress_interval)
