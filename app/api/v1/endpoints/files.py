...
from app.schemas.files import (FileDeleteResponse, FileListResponse,
...
async def upload_file(
  ...
  if metadata:
    try:
      ...
      raise HTTPException(
        status.HTTP_400_BAD_REQUEST, detail="metadata must be valid JSON"
      ) from exc
...
async def list_files(
  ...
  payload = [
    FileMetadataResponse.model_validate(record, from_attributes=True) for record in records
  ]
...
async def delete_file(
  ...
  return FileDeleteResponse(
    id=file_id, filename=record.filename, message="File deleted successfully"
  )
