...
class WebSocketManager:
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            logger.debug(
                "WebSocketManager stopped without running loop; skipping cleanup scheduling."
            )
            return
    ...
    async def send_personal_message(self, message: dict[str, Any], websocket: WebSocket) -> None:
        ...
        try:
            ...
            await self.disconnect(
                websocket, self._connection_meta.get(websocket, {}).get("session_id")
            )
    ...
    async def broadcast_to_session(self, session_id: int, message: dict[str, Any]) -> None:
        ...
        for connection in connections:
            try:
                ...
                logger.critical(
                    "Unexpected error broadcasting to session %s", session_id, exc_info=exc
                )
                stale_connections.append(connection)
    ...
    async def broadcast_all(self, message: dict[str, Any]) -> None:
        ...
        for connection in stale_connections:
            await self.disconnect(
                connection, self._connection_meta.get(connection, {}).get("session_id")
            )
