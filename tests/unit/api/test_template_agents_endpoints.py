...
class TestListAgentTypes:
  ...
  def test_horizontal_scan_type(self) -> None:
    ...
    horizontal = next((item for item in result if item["type"] == "horizontal_scan"), None)
...
class TestCompareAgents:
  ...
  def test_performance_gap_calculation(self) -> None:
    ...
    expected_gap = response.results[0].average_reward - response.results[-1].average_reward
