...
class FakePubSub:
  ...
  async def get_message(
    self, *, timeout: float | None = None, ignore_subscribe_messages: bool = True
  ) -> dict[str, object] | None:  # noqa: D401
...
async def test_forwarder_broadcasts_messages_to_active_session() -> None:
  ...
  forwarder = RedisTrainingEventForwarder(
    redis_client=redis, websocket_manager=manager, poll_interval=0.01
  )
