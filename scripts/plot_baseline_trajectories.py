...
def plot_agent_subplots(agent_name, episodes):
    ...
    for i, ax in enumerate(axes):
        if i < len(episodes):
            ...
            ax.set_xticks([]) # Hide ticks for cleanliness in small plots
            ax.set_yticks([])
            # ax.grid(True, color='lightgray', linestyle='--', alpha=0.3) # Grid might clutter
            ax.set_title(f"Ep {ep_data[0]['episode']}", fontsize=8)
            ax.set_aspect('equal')
            ax.invert_yaxis()
            
            # Draw Obstacles (from first frame of ep)
            obs = ep_data[0].get('obstacles', [])
            if obs and isinstance(obs, list) and isinstance(obs[0], list):
                 ox = []
                 oy = []
                 for y, row in enumerate(obs):
                     for x, is_obs in enumerate(row):
                         if is_obs:
                             ox.append(x)
                             oy.append(y)
                 ax.scatter(ox, oy, c='black', marker='s', s=10) # Smaller marker
            
            # Draw Path
            xs = [r['robot_x'] for r in ep_data]
            ys = [r['robot_y'] for r in ep_data]
            ax.plot(xs, ys, color='blue', alpha=0.7, linewidth=0.8)
    ...
    plt.suptitle(f"{agent_name} Trajectories (50 Episodes)", fontsize=16)
    plt.tight_layout()
    output_path = f"{OUTPUT_DIR}/baseline_trajectories_{agent_name.lower()}_50grid.png"
...
def plot_baseline_trajectories():
    # Load All
    zigzag_eps = load_all_eps("trajectory_zigzag.jsonl")
    spiral_eps = load_all_eps("trajectory_spiral.jsonl")
    
    plot_agent_subplots("Zigzag", zigzag_eps)
    plot_agent_subplots("Spiral", spiral_eps)
