"""
Multi-agent trajectory visualization using direct database query.
...
sys.path.insert(0, "/home/hama/work/master/security-robot-be")

from sqlalchemy import create_engine, text
from sqlalchemy.orm import sessionmaker

# Database connection
DATABASE_URL = "postgresql://postgres:postgres@localhost:5432/security_robot"
...
def generate_trajectory_figures():
  """Generate trajectory visualization figures using database data."""
  ...
  for i, num_robots in enumerate(robot_counts):
    ...
    for j, (episode, stage_name) in enumerate(EPISODE_STAGES):
      ...
      if frames:
        ...
        print(
          f"  Episode {episode}: {len(frames)} frames, trajectories: {[len(t) for t in trajectories]}"
        )
      else:
