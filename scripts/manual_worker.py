...
async def process_job(job_id: int) -> None:
  ...
  try:
    ...
    db.execute(
      text("UPDATE trainingjob SET status = 'running', started_at = NOW() WHERE id = :id"),
      {"id": job_id},
    )
    db.commit()
    ...
    def _status_probe() -> TrainingJobStatus:
    ...
    training_result = await service.start_training(
      config=config,
      callbacks=callbacks,
      session_id=job_id,
    ...
    logger.info(f"Job {job_id} Finished: {training_result.get('status')}")
    ...
    db.execute(
      text("UPDATE trainingjob SET status = :status, completed_at = NOW() WHERE id = :id"),
      {"status": completion_status, "id": job_id},
    )
    db.commit()
    ...
    try:
      db.execute(
        text("UPDATE trainingjob SET status = 'failed', completed_at = NOW() WHERE id = :id"),
        {"id": job_id},
      )
      db.commit()
    except Exception:
