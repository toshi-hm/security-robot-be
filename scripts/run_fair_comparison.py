...
def make_env(num_robots):
    def _init():
        return SecurityEnvironment(
            ...
            exploration_bonus=1.0, revisit_penalty=0.05, revisit_window=50,
        )
    return _init
...
def evaluate_template(AgentClass, num_robots, num_episodes=10):
    ...
    for ep in range(num_episodes):
        ...
        obstacle_set = {(x, y) for y in range(env.height) for x in range(env.width) if env.obstacles[y][x]}
...
def main():
    ...
    print('-'*60)
    print(f"{'PPO':<15} {ppo_results['mean_reward']:>8.2f} ± {ppo_results['std_reward']:<8.2f} "
          f"{ppo_results['mean_coverage']*100:>6.2f}% ± {ppo_results['std_coverage']*100:<5.2f}%")
    print(f"{'Spiral':<15} {spiral_results['mean_reward']:>8.2f} ± {spiral_results['std_reward']:<8.2f} "
          f"{spiral_results['mean_coverage']*100:>6.2f}% ± {spiral_results['std_coverage']*100:<5.2f}%")
    print(f"{'Zigzag':<15} {zigzag_results['mean_reward']:>8.2f} ± {zigzag_results['std_reward']:<8.2f} "
    ...
    with open(results_file, 'w', newline='') as f:
        ...
        writer.writerow(['PPO', ppo_results['mean_reward'], ppo_results['std_reward'],
                        ppo_results['mean_coverage'], ppo_results['std_coverage']])
        writer.writerow(['Spiral', spiral_results['mean_reward'], spiral_results['std_reward'],
                        spiral_results['mean_coverage'], spiral_results['std_coverage']])
        writer.writerow(['Zigzag', zigzag_results['mean_reward'], zigzag_results['std_reward'],
                        zigzag_results['mean_coverage'], zigzag_results['std_coverage']])
