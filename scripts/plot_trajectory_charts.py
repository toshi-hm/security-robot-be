...
def plot_threat_transition_single(ep_target=47):
    ...
    plt.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/thesis_single_threat_transition_new.svg")
    plt.savefig(f"{OUTPUT_DIR}/thesis_single_threat_transition_new.png")
    plt.close()
...
def plot_trajectories(n_robots, target_episodes, output_name):
    ...
    for idx, ep in enumerate(target_episodes):
        ...
        for r_id in range(n_robots):
            ...
            # End (Square) - Removed as per request
             ...
             for y, row in enumerate(obs):
                 for x, is_obs in enumerate(row):
                     if is_obs:
                         ox.append(x)
                         oy.append(y)
             ax.scatter(ox, oy, c='black', marker='s', s=120)
    
    plt.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/{output_name}_new.svg")
    plt.savefig(f"{OUTPUT_DIR}/{output_name}_new.png")
    plt.close()
...
if __name__ == "__main__":
    ...
    # 2. Single Agent Trajectories (Ep 1, 2, 25, 49, 50)
    plot_trajectories(1, [1, 2, 25, 49, 50], "thesis_single_trajectories_v3")
