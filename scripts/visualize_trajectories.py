"""
Multi-agent trajectory visualization using real playback data from latest experiments.
Creates trajectory plots for 2, 3, and 4 robot configurations
at different training stages (early, mid, late episodes).
Uses Jobs 72-74 which are the experiments reported in the thesis.
Uses English labels to avoid font issues.
"""
...
# Episodes to visualize: early (1,2), mid (25), late (49,50)
EPISODE_STAGES = [
  (1, "ep 1"),
  (2, "ep 2"),
  (25, "ep 25"),
...
def fetch_frames(session_id: int, episode: int, limit: int = 1000) -> list:
  """Fetch playback frames from API."""
  cmd = f'curl -s "http://localhost:8000/api/v1/playback/{session_id}/frames?episode={episode}&limit={limit}"'
  result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
  try:
    data = json.loads(result.stdout)
    # API returns {'total', 'page', 'page_size', 'frames'}
...
def extract_trajectories(frames: list, num_robots: int) -> tuple:
  ...
  for frame in frames:
    ...
    if not obstacles:
      obs_grid = frame.get("obstacles", {}).get("levels", [])
...
def generate_trajectory_figures():
  """Generate trajectory visualization figures using playback data."""
  ...
  # Generate individual figures for each robot count (horizontal layout)
  for num_robots in robot_counts:
    session_id = SESSIONS[num_robots]
    fig, axes = plt.subplots(1, 5, figsize=(14, 2.8), dpi=150)

    for i, (episode, stage_name) in enumerate(EPISODE_STAGES):
      ax = axes[i]
      frames = fetch_frames(session_id, episode)

      if frames:
        trajectories, obstacles = extract_trajectories(frames, num_robots)
        plot_trajectory(ax, trajectories, obstacles, stage_name)

    plt.suptitle(f"Trajectory evolution ({num_robots} robots)", fontsize=11, fontweight="bold")
    plt.tight_layout()
    output_path = f"{OUTPUT_DIR}/trajectory_{num_robots}robots.png"
    plt.savefig(output_path, bbox_inches="tight", facecolor="white")
    plt.close()
    print(f"Saved: {output_path}")
