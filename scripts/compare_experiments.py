...
async def get_metrics(client, job_id):
  try:
    ...
    response = await client.get(
      f"{API_URL_BASE}/training/sessions/{job_id}/metrics?page=1&page_size=50"
    )
    if response.status_code != 200:
      return []
...
async def main():
  ...
  async with httpx.AsyncClient() as client:
    for job_id in job_ids:
      ...
      threat_vals = [
        m["threat_level_avg"] for m in last_metrics if m["threat_level_avg"] is not None
      ]
  ...
  print("\n" + "=" * 95)
  print(
    f"{'ID':<4} | {'Robots':<6} | {'Mode':<6} | {'Cov':<6} | {'Threat':<6} | {'Reward':<8} | {'TeamRew':<8} | {'Name':<30}"
  )
  print("-" * 95)

  for r in results:
    print(
      f"{r['id']:<4} | {r['robots']:<6} | {r['mode']:<6} | {r['cov']:.4f} | {r['threat']:.4f} | {r['reward']:<8.1f} | {r['team_reward']:<8.1f} | {r['name']}"
    )
